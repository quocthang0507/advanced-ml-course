...
class DuelingDQN(nn.Module):
    def __init__(self, action_dim):
        ...
        self.conv_out_size = self._get_conv_out((84, 84))

        self.value_fc = nn.Linear(self.conv_out_size, 512)
        self.value = nn.Linear(512, 1)

        self.advantage_fc = nn.Linear(self.conv_out_size, 512)
        self.advantage = nn.Linear(512, action_dim)

    def _get_conv_out(self, shape):
        x = torch.zeros(1, 4, shape[0], shape[1])
...
class DQNAgent:
    def __init__(self, device, action_dim):
        self.device = device
        self.q_network = DuelingDQN(action_dim).to(self.device)
        self.target_network = DuelingDQN(action_dim).to(self.device)
    ...
    def train(self):
        self.step_count += 1
        if self.step_count < self.start_training or self.step_count % self.train_every != 0:
            return
