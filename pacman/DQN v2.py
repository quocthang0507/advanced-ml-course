...
class DQN(nn.Module):
    def __init__(self, action_dim):
        ...
        self.conv_out_size = self._get_conv_out((84, 84)) #hardcoded 84x84
    ...
    def _get_conv_out(self, shape):
        ...
        x = torch.zeros(1, 4, shape[0], shape[1])  # Assume input shape is (84, 84) and 4 input channels
        x = torch.relu(self.conv1(x))
        x = torch.relu(self.conv2(x))
        x = torch.relu(self.conv3(x))
        x = torch.relu(self.conv4(x)) # Pass through the new conv layer
